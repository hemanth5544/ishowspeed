...
function GitHubStarsButton({
  ...
  const [stars, setStars] = React.useState<number | null>(starsProp ?? null)
  const [fullName, setFullName] = React.useState(`${owner}/${repo}`)
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      ...
      setFullName(data?.fullName ?? `${owner}/${repo}`)
    })()

    return () => {
      cancelled = true
    }
  }, [owner, repo, starsProp])
